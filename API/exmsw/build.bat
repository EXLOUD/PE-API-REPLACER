...
REM Пошук exws2.lib у кінцевій папці
...
if defined EXWS2_LIB (
    ...
    "%LLVM%\%1-gcc" %FLAGS% %3 -o "%~dp0%DLL%" "%~dp0dllmain.c" "%~dp0%RES%" "%~dp0exmsw.def" "%EXWS2_LIB%"
) else (
    "%LLVM%\%1-gcc" %FLAGS% %3 -o "%~dp0%DLL%" "%~dp0dllmain.c" "%~dp0%RES%" "%~dp0exmsw.def"
)
