    ...
    switch (ul_reason_for_call) {
        case DLL_PROCESS_ATTACH:
            ...
            if (AllocConsole()) {
                ...
                wprintf(L"=========================================================\n");
                wprintf(L"    NetAPI32 Stub Debug Console v1.0\n");
                wprintf(L"    Build: %S %S\n", __DATE__, __TIME__);
                wprintf(L"=========================================================\n\n");
...
NET_API_STATUS WINAPI ex_NetWkstaGetInfo(LMSTR servername, DWORD level, LPBYTE* bufptr) {
    LogMessage(L"NetWkstaGetInfo(servername: %s, level: %lu)", servername ? servername : L"(null)", level);
    ...
    if (err != NERR_Success) { 
        LogMessage(L"  -> NetWkstaGetInfo returning %lu", err); 
        return err; 
    }
    ...
    *bufptr = (LPBYTE)info;
    LogMessage(L"  -> NetWkstaGetInfo returning NERR_Success");
    return NERR_Success;
}
...
NET_API_STATUS WINAPI ex_NetWkstaUserGetInfo(LMSTR r, DWORD l, LPBYTE* b) { 
    LogMessage(L"NetWkstaUserGetInfo(level: %lu)", l); 
    return ERROR_ACCESS_DENIED; 
}
...
NET_API_STATUS WINAPI ex_NetServerGetInfo(LMSTR s, DWORD l, LPBYTE *b) { 
    LogMessage(L"NetServerGetInfo(servername: %s, level: %lu)", s ? s : L"(null)", l); 
    if (l == 101) { 
        DWORD er, te; 
        return ex_NetServerEnum(s, l, b, MAX_PREFERRED_LENGTH, &er, &te, 0, NULL, NULL); 
    } 
...
NET_API_STATUS WINAPI ex_NetShareEnum(LMSTR s, DWORD l, LPBYTE* b, DWORD p, LPDWORD er, LPDWORD te, LPDWORD rh) {
    LogMessage(L"NetShareEnum(servername: %s, level: %lu)", s ? s : L"(null)", l);
    if (!IsLocalComputer(s)) return NERR_InvalidComputer; 
...
DWORD WINAPI ex_NetGetJoinInformation(LPCWSTR s, LPWSTR* n, PNETSETUP_JOIN_STATUS bt) {
    LogMessage(L"NetGetJoinInformation(server: %s)", s ? s : L"(null)");
    if (!IsLocalComputer(s)) return NERR_InvalidComputer; 
    if (!bt) return ERROR_INVALID_PARAMETER;
    
    WCHAR domain[DNLEN + 1] = L"WORKGROUP"; 
    DWORD size = ARRAYSIZE(domain);
    NETSETUP_JOIN_STATUS status = NetSetupWorkgroupName;
    
    if (GetComputerNameExW(ComputerNameDnsDomain, domain, &size) && size > 1) {
        status = NetSetupDomainName;
    }
    
    NET_API_STATUS err = ex_NetApiBufferAllocate((wcslen(domain) + 1) * sizeof(WCHAR), (LPVOID*)n); 
    if (err != NERR_Success) return err;
    
    wcscpy_s(*n, wcslen(domain) + 1, domain); 
    *bt = status; 
    LogMessage(L"  -> Domain/Workgroup: %s, Status: %d", domain, status);
    return NERR_Success;
}
...
UCHAR WINAPI ex_Netbios(PNCB pncb) { 
    LogMessage(L"Netbios(...)"); 
    if (pncb) pncb->ncb_retcode = NRC_ILLCMD; 
    return NRC_ILLCMD; 
}
...
NET_API_STATUS WINAPI ex_NetUserEnum(LPCWSTR s, DWORD l, DWORD f, LPBYTE* b, DWORD p, LPDWORD er, LPDWORD te, LPDWORD rh) { 
    LogMessage(L"NetUserEnum(servername: %s, level: %lu)", s ? s : L"(null)", l); 
    if (!IsLocalComputer(s)) return NERR_InvalidComputer; 
    if (l > 3 && l != 10 && l != 11 && l != 20 && l != 23) return ERROR_INVALID_LEVEL; 
    if (!b||!er||!te) return ERROR_INVALID_PARAMETER; 
    *er = *te = 0; *b = NULL; 
    return NERR_Success; 
}
