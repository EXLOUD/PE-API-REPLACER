// ============================================================================
// === WSOCK32 Proxy & MSWSOCK Stub v1.0.1 ===
// === Автор: EXLOUD ===
...
VOID PASCAL FAR ex_GetAcceptExSockaddrs(PVOID lpOutputBuffer, DWORD dwReceiveDataLength, DWORD dwLocalAddressLength, DWORD dwRemoteAddressLength, LPSOCKADDR* LocalSockaddr, LPINT LocalSockaddrLength, LPSOCKADDR* RemoteSockaddr, LPINT RemoteSockaddrLength) {
    ...
    generic_addr.sin_addr.s_addr = 0x0100007F; // 127.0.0.1 in network byte order
...
// === Прямі реалізації для сумісності з системним wsock32.dll ===
...
int WSAAPI ex_getsockopt(SOCKET s, int level, int optname, char* optval, int* optlen) {
    static LPFN_GETSOCKOPT pfn = NULL;
    if (!pfn) pfn = (LPFN_GETSOCKOPT)GetProcAddress(GetModuleHandleA("exws2"), "getsockopt");
    if (pfn) return pfn(s, level, optname, optval, optlen);
    SetMSWSockError(WSAENOPROTOOPT);
    return SOCKET_ERROR;
}

int WSAAPI ex_recv(SOCKET s, char* buf, int len, int flags) {
    static LPFN_RECV pfn = NULL;
    if (!pfn) pfn = (LPFN_RECV)GetProcAddress(GetModuleHandleA("exws2"), "recv");
    if (pfn) return pfn(s, buf, len, flags);
    SetMSWSockError(WSAENOTCONN);
    return SOCKET_ERROR;
}

int WSAAPI ex_recvfrom(SOCKET s, char* buf, int len, int flags, struct sockaddr* from, int* fromlen) {
    static LPFN_RECVFROM pfn = NULL;
    if (!pfn) pfn = (LPFN_RECVFROM)GetProcAddress(GetModuleHandleA("exws2"), "recvfrom");
    if (pfn) return pfn(s, buf, len, flags, from, fromlen);
    SetMSWSockError(WSAENOTCONN);
    return SOCKET_ERROR;
}

int WSAAPI ex_setsockopt(SOCKET s, int level, int optname, const char* optval, int optlen) {
    static LPFN_SETSOCKOPT pfn = NULL;
    if (!pfn) pfn = (LPFN_SETSOCKOPT)GetProcAddress(GetModuleHandleA("exws2"), "setsockopt");
    if (pfn) return pfn(s, level, optname, optval, optlen);
    SetMSWSockError(WSAENOPROTOOPT);
    return SOCKET_ERROR;
}
    ...
    switch (ul_reason_for_call)
    {
        case DLL_PROCESS_ATTACH:
        {
            if (InterlockedIncrement(&g_InitCount) == 1) {
                ...
                #endif
                LogMessage("=== MSWSOCK Stub v1.0.1 Initialized ===");
            }
            break;
        }
        case DLL_PROCESS_DETACH:
        {
            if (InterlockedDecrement(&g_InitCount) == 0) {
                LogMessage("=== MSWSOCK Stub v1.0.1 Unloading ===");
                if (g_tlsError != TLS_OUT_OF_INDEXES) {
                    TlsFree(g_tlsError);
                }
                #if ENABLE_FILE_LOGGING
