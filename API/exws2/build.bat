...
set "DLL=exws2.dll"
set "LIB=exws2.lib"
set "FLAGS=-shared -O3 -flto -ffunction-sections -fdata-sections  -s -static-libgcc -Wl,--enable-stdcall-fixup -Wl,--gc-sections -Wl,--strip-all -lkernel32"
...
"%LLVM%\%1-gcc" %FLAGS% -Wl,--output-def,"%~dp0%LIB:.lib=.def%" -o "%~dp0%DLL%" "%~dp0dllmain.c" "%~dp0%RES%" "%~dp0ws2_32.def"
if errorlevel 1 (
...
"%LLVM%\%1-dlltool" -d "%~dp0%LIB:.lib=.def%" -l "%~dp0%LIB%" -D "%DLL%"
if errorlevel 1 (
    echo ERROR: dlltool failed for %RUNTIME%\%2
    del "%~dp0%RES%" 2>nul
    del "%~dp0%DLL%" 2>nul
    del "%~dp0%LIB:.lib=.def%" 2>nul
    pause
    exit /b 1
)
copy /Y "%~dp0%DLL%" "%~dp0%RUNTIME%\%2\" >nul 2>nul
copy /Y "%~dp0%LIB%" "%~dp0%RUNTIME%\%2\" >nul 2>nul
del "%~dp0%DLL%" 2>nul
del "%~dp0%LIB%" 2>nul
del "%~dp0%LIB:.lib=.def%" 2>nul
del "%~dp0%RES%" 2>nul
