// =========================================================================================
// Winsock2 Emulator v1.0.3 by EXLOUD
//
// - Core Architecture: Inter-process communication via Shared Memory
// - Protocol Support: IPv4/IPv6 for TCP (SOCK_STREAM) and UDP (SOCK_DGRAM)
...
// --- Configuration ---
#define ENABLE_CONSOLE            0 // [ВИМКНЕНО]
#define ENABLE_FILE_LOG           0 // [ВИМКНЕНО]
#define ENABLE_DATA_DUMP          0 // [ВИМКНЕНО]
#define ENABLE_BEHAVIOR_ANALYSIS  0 // [ВИМКНЕНО]
#define ENABLE_MEMORY_TRACKING    0 // [ВИМКНЕНО]
...
static const char* SHARED_MEM_NAME = "Local\\EXLOUD_VirtualNetworkState_v1_0_3";
static const char* MUTEX_NAME      = "Local\\EXLOUD_VirtualNetworkMutex_v1_0_3";
...
typedef struct {
    BOOL in_use, is_listening, is_connected, is_bound, is_nonblocking; SOCKET fake_socket_id; DWORD owner_pid;
    int sock_type; // SOCK_STREAM або SOCK_DGRAM
    int shutdown_flags, bind_addr_len, peer_socket_index, accept_queue[MAX_PENDING_CONN], accept_queue_len;
...
static void AutoBindIfNeeded(SHARED_SOCKET* s, int family, int type);
...
// --- Memory Tracking ---
// [ВИМКНЕНО] Функції відстеження пам'яті будуть замінені на стандартні
#if ENABLE_MEMORY_TRACKING
...
// [ВИМКНЕНО] Функції логування будуть пустими
#if ENABLE_CONSOLE || ENABLE_FILE_LOG
#if !ENABLE_CONSOLE
...
#if ENABLE_CONSOLE
    if (!g_hConsole && g_init_count == 0) return;
#else
    if (!g_hLogFile && g_init_count == 0) return;
#endif
    char buffer[2048]; va_list args; va_start(args, format); _vsnprintf_s(buffer, sizeof(buffer), _TRUNCATE, format, args); va_end(args); char out[2300]; _snprintf_s(out, sizeof(out), _TRUNCATE, "[PID:%05lu TID:%05lu] %s", GetCurrentProcessId(), GetCurrentThreadId(), buffer);
#if ENABLE_CONSOLE
    if (g_hConsole) { DWORD w; WriteConsoleA(g_hConsole, out, (DWORD)strlen(out), &w, NULL); WriteConsoleA(g_hConsole, "\r\n", 2, &w, NULL); }
#else
    OutputDebugStringA(out); InternalLog(g_hLogFile, out, TRUE);
#endif
}
#else
...
static void AutoBindIfNeeded(SHARED_SOCKET* s, int family, int type) {
    ...
    addr.ss_family = (short)family;
    if (type == SOCK_STREAM) { // Auto-bind only for connection-oriented sockets
        if (family == AF_INET) {
            struct sockaddr_in* a = (struct sockaddr_in*)&addr;
            a->sin_family = AF_INET;
            a->sin_addr.s_addr = ex_inet_addr("127.0.0.1");
            while (1) { unsigned short np = GetNextEphemeralPort(); if (!PortInUse(np, AF_INET)) { a->sin_port = ex_htons(np); break; } }
            s->bind_addr_len = sizeof(struct sockaddr_in);
        } else {
            struct sockaddr_in6* a6 = (struct sockaddr_in6*)&addr;
            a6->sin6_family = AF_INET6; a6->sin6_addr = in6addr_loopback;
            while (1) { unsigned short np = GetNextEphemeralPort(); if (!PortInUse(np, AF_INET6)) { a6->sin6_port = ex_htons(np); break; } }
            s->bind_addr_len = sizeof(struct sockaddr_in6);
        }
        memcpy(&s->bind_addr, &addr, sizeof(addr));
        s->is_bound = TRUE;
    }
}
...
#if ENABLE_CONSOLE
    if (!g_hConsole) { AllocConsole(); g_hConsole = GetStdHandle(STD_OUTPUT_HANDLE); }
#elif ENABLE_FILE_LOG
...
#endif
LogMessage("--- EXLOUD Winsock Emulator v1.0.3 Initializing ---");
...
ReportMemoryLeaks();
#endif
if(g_pGlobalState)UnmapViewOfFile(g_pGlobalState);if(g_hSharedMemory)CloseHandle(g_hSharedMemory);if(g_hGlobalMutex)CloseHandle(g_hGlobalMutex);g_pGlobalState=NULL;g_hSharedMemory=NULL;g_hGlobalMutex=NULL;
#if !ENABLE_CONSOLE && ENABLE_FILE_LOG
if(g_hLogFile){CloseHandle(g_hLogFile);g_hLogFile=NULL;}
#if ENABLE_DATA_DUMP
...
int WSAAPI ex_WSAStartup(WORD w, LPWSADATA d) { if(InterlockedIncrement(&g_init_count) == 1) InitializeEmulator(); LogMessage("WSAStartup"); if (d) { d->wVersion=MAKEWORD(2,2); d->wHighVersion=MAKEWORD(2,2); strcpy_s(d->szDescription, sizeof(d->szDescription), "EXLOUD Winsock Emulator v1.0.3"); } return 0; }
...
SOCKET WSAAPI ex_socket(int af, int t, int p) {
    EnsureInitialized();
    LogMessage("socket(af=%d, type=%d, proto=%d)", af, t, p);
    if (t != SOCK_STREAM && t != SOCK_DGRAM) {
        g_tlsWSAError = WSAESOCKTNOSUPPORT;
        return INVALID_SOCKET;
    }
    if (af != AF_INET && af != AF_INET6) {
        g_tlsWSAError = WSAEAFNOSUPPORT;
        return INVALID_SOCKET;
    }
    Lock();
    int i = FindFreeSocketSlot();
    if (i == -1) { Unlock(); g_tlsWSAError = WSAENOBUFS; return INVALID_SOCKET; }
    SHARED_SOCKET* s = &g_pGlobalState->sockets[i];
    memset(s, 0, sizeof(SHARED_SOCKET));
    s->in_use = TRUE;
    s->owner_pid = GetCurrentProcessId();
    s->fake_socket_id = GenerateFakeSocket();
    s->sock_type = t;
    s->peer_socket_index = -1;
    s->bind_addr.ss_family = (short)af;
    s->bind_addr_len = (af == AF_INET) ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6);
    SOCKET new_id = s->fake_socket_id;
    Unlock();
    LogMessage(" -> " PTR_FORMAT, (UINT_PTR)new_id);
    return new_id;
}
...
int WSAAPI ex_listen(SOCKET s, int b) {
    EnsureInitialized();
    LogMessage("listen(" PTR_FORMAT ")", (UINT_PTR)s);
    Lock();
    SHARED_SOCKET* sock = FindSharedSocketById(s);
    if (!sock || !sock->is_bound) { Unlock(); g_tlsWSAError = sock ? WSAEINVAL : WSAENOTSOCK; return SOCKET_ERROR; }
    if (sock->sock_type != SOCK_STREAM) { Unlock(); g_tlsWSAError = WSAEOPNOTSUPP; return SOCKET_ERROR; }
    sock->is_listening = TRUE;
    Unlock();
    return 0;
}
int WSAAPI ex_connect(SOCKET s, const struct sockaddr* n, int nl) {
    EnsureInitialized(); if (!n || nl < (int)sizeof(struct sockaddr)) { g_tlsWSAError = WSAEFAULT; return SOCKET_ERROR; }
    int f = n->sa_family; if (f != AF_INET && f != AF_INET6) { g_tlsWSAError = WSAEAFNOSUPPORT; return SOCKET_ERROR; }
    unsigned short p = GetSockaddrPort(n); LogMessage("connect(" PTR_FORMAT ") to port %u", (UINT_PTR)s, p);
    Lock();
    SHARED_SOCKET* c = FindSharedSocketById(s);
    if (!c) { Unlock(); g_tlsWSAError = WSAENOTSOCK; return SOCKET_ERROR; }
    if (c->sock_type != SOCK_STREAM) { Unlock(); g_tlsWSAError = WSAEOPNOTSUPP; return SOCKET_ERROR; }
    AutoBindIfNeeded(c, f, SOCK_STREAM);
    int l = -1;
    for (int i = 0; i < MAX_SHARED_SOCKETS; i++) { SHARED_SOCKET* ls = &g_pGlobalState->sockets[i]; if (ls->in_use && ls->is_listening && ls->bind_addr.ss_family == f && GetSockaddrPort((struct sockaddr*)&ls->bind_addr) == p) { l = i; break; } }
    if (l == -1) { Unlock(); LogMessage(" -> ECONNREFUSED"); g_tlsWSAError = WSAECONNREFUSED; return SOCKET_ERROR; }
    SHARED_SOCKET* srv = &g_pGlobalState->sockets[l]; if (srv->accept_queue_len >= MAX_PENDING_CONN) { Unlock(); LogMessage(" -> ECONNREFUSED (queue full)"); g_tlsWSAError = WSAECONNREFUSED; return SOCKET_ERROR; }
    int si = FindFreeSocketSlot(); if (si == -1) { Unlock(); g_tlsWSAError = WSAENOBUFS; return SOCKET_ERROR; }
    int ci = (int)(c - g_pGlobalState->sockets);
    SHARED_SOCKET* ss = &g_pGlobalState->sockets[si];
    memset(ss, 0, sizeof(SHARED_SOCKET));
    ss->in_use = TRUE; ss->owner_pid = srv->owner_pid; ss->fake_socket_id = GenerateFakeSocket(); ss->is_connected = TRUE; ss->peer_socket_index = ci; ss->is_bound = TRUE; ss->bind_addr = srv->bind_addr; ss->bind_addr_len = srv->bind_addr_len; ss->sock_type = SOCK_STREAM;
    c->is_connected = TRUE; c->peer_socket_index = si;
    srv->accept_queue[srv->accept_queue_len++] = si;
    unsigned short lp = GetSockaddrPort((struct sockaddr*)&srv->bind_addr); int fam = srv->bind_addr.ss_family;
    Unlock();
    char en[96]; MakeAcceptEventName(en, sizeof(en), fam, lp);
    HANDLE h = OpenEventA(EVENT_MODIFY_STATE, FALSE, en);
    if (h) { LogMessage(" -> Signaling accept"); SetEvent(h); CloseHandle(h); }
    LogMessage(" -> Connected " PTR_FORMAT " <=> " PTR_FORMAT, (UINT_PTR)c->fake_socket_id, (UINT_PTR)ss->fake_socket_id);
    return 0;
}
SOCKET WSAAPI ex_accept(SOCKET s, struct sockaddr* a, int* al) {
    EnsureInitialized(); LogMessage("accept(" PTR_FORMAT ")", (UINT_PTR)s);
    char en[96]; HANDLE h = NULL;
    Lock();
    SHARED_SOCKET* l = FindSharedSocketById(s);
    if (!l || !l->is_listening) { Unlock(); g_tlsWSAError = WSAEINVAL; return INVALID_SOCKET; }
    if (l->sock_type != SOCK_STREAM) { Unlock(); g_tlsWSAError = WSAEOPNOTSUPP; return INVALID_SOCKET; }
    unsigned short p = GetSockaddrPort((struct sockaddr*)&l->bind_addr); int fam = l->bind_addr.ss_family;
    Unlock();
    MakeAcceptEventName(en, sizeof(en), fam, p); h = CreateEventA(NULL, FALSE, FALSE, en);
    if (!h) { g_tlsWSAError = WSAENETDOWN; return INVALID_SOCKET; }
    while (1) {
        Lock(); l = FindSharedSocketById(s);
        if (l && l->accept_queue_len > 0) {
            int si = l->accept_queue[0];
            for (int i = 0; i < l->accept_queue_len - 1; i++)l->accept_queue[i] = l->accept_queue[i + 1];
            l->accept_queue_len--;
            SHARED_SOCKET* ss = &g_pGlobalState->sockets[si];
            SHARED_SOCKET* cs = &g_pGlobalState->sockets[ss->peer_socket_index];
            if (a && al && *al >= (int)sizeof(struct sockaddr)) { int cl = min(*al, cs->bind_addr_len); memcpy(a, &cs->bind_addr, cl); *al = cl; }
            Unlock(); CloseHandle(h); LogMessage(" -> Accepted " PTR_FORMAT, (UINT_PTR)ss->fake_socket_id); return ss->fake_socket_id;
        }
        BOOL nb = l ? l->is_nonblocking : FALSE;
        Unlock(); if (nb) { CloseHandle(h); g_tlsWSAError = WSAEWOULDBLOCK; return INVALID_SOCKET; }
        LogMessage(" -> Waiting on accept..."); WaitForSingleObject(h, INFINITE);
    }
}
...
int WSAAPI ex_send(SOCKET s, const char* b, int l, int f) {
    ...
    if (l <= 0) return 0;
    HANDLE wh = NULL; char wen[96];
retry:
    Lock();
    SHARED_SOCKET* snd = FindSharedSocketById(s);
    if (!snd) { Unlock(); if (wh) CloseHandle(wh); g_tlsWSAError = WSAENOTSOCK; return SOCKET_ERROR; }
    if (snd->sock_type != SOCK_STREAM) { Unlock(); if(wh) CloseHandle(wh); g_tlsWSAError = WSAEOPNOTSUPP; return SOCKET_ERROR; }
    ...
    if (tw > 0) {
        ...
        char* buf = GetRecvBuffer(rcv); size_t tail = rcv->recv_tail; size_t cap = recv_capacity();
        ...
        #if ENABLE_BEHAVIOR_ANALYSIS
        snd->behavior.total_bytes_sent += tw; if (tw == 1) snd->behavior.single_byte_count++; else if (tw < 16) snd->behavior.short_messages++; else snd->behavior.long_messages++;
        #endif
        Unlock();
        #if ENABLE_DATA_DUMP
        DumpDataToFile("SEND", s, b, tw);
        #endif
        char ren[96]; MakeReadEventName(ren, sizeof(ren), rid); HANDLE rh = OpenEventA(EVENT_MODIFY_STATE, FALSE, ren); if (rh) { SetEvent(rh); CloseHandle(rh); }
...
int WSAAPI ex_recv(SOCKET s, char* b, int l, int f) {
    EnsureInitialized();
    if (l <= 0) return 0;
    char ren[96]; MakeReadEventName(ren, sizeof(ren), s); HANDLE h = CreateEventA(NULL, FALSE, FALSE, ren);
    ...
    while (1) {
        ...
        if (rcv->sock_type != SOCK_STREAM) { Unlock(); CloseHandle(h); g_tlsWSAError = WSAEOPNOTSUPP; return SOCKET_ERROR; }
        ...
        if (avail > 0) {
            ...
            char* buf = GetRecvBuffer(rcv); size_t head = rcv->recv_head; size_t cap = recv_capacity();
            size_t first = min((size_t)tr, cap - head);
            memcpy(b, buf + head, first);
            if ((size_t)tr > first) memcpy(b + first, buf, tr - first);
            int pi = rcv->peer_socket_index; SOCKET pid = INVALID_SOCKET;
            if (!(f & MSG_PEEK)) {
            ...
            Unlock(); CloseHandle(h);
            #if ENABLE_DATA_DUMP
            if (!(f & MSG_PEEK)) DumpDataToFile("RECV", s, b, tr);
            #endif
            if (!(f & MSG_PEEK) && pid != INVALID_SOCKET) { char wen[96]; MakeWriteEventName(wen, sizeof(wen), pid); HANDLE wh = OpenEventA(EVENT_MODIFY_STATE, FALSE, wen); if (wh) { SetEvent(wh); CloseHandle(wh); } }
            LogMessage("recv(" PTR_FORMAT ", len=%d) -> read %d", (UINT_PTR)s, l, tr);
            return tr;
        }
        if (rcv->peer_socket_index != -1) { SHARED_SOCKET* p = &g_pGlobalState->sockets[rcv->peer_socket_index]; if ((p->shutdown_flags & 2) != 0 || !p->in_use) { Unlock(); CloseHandle(h); return 0; }
        } else if (rcv->is_connected) { /* Peer closed gracefully */ Unlock(); CloseHandle(h); return 0;
        } else { /* Not connected at all */ Unlock(); CloseHandle(h); g_tlsWSAError = WSAENOTCONN; return SOCKET_ERROR; }
        BOOL nb = rcv->is_nonblocking; Unlock();
...
int WSAAPI ex_gethostname(char*n,int l){if(n&&l>0){strcpy_s(n,l,"EXLOUD-EMU");return 0;}g_tlsWSAError=WSAEFAULT;return SOCKET_ERROR;}
...
int WSAAPI ex_getaddrinfo(PCSTR n,PCSTR s,const ADDRINFOA*h,PADDRINFOA*r){if(!r)return EAI_FAIL;*r=NULL;BOOL l=(n==NULL)||(_stricmp(n,"localhost")==0);BOOL v4=n&&(_stricmp(n,"127.0.0.1")==0);BOOL v6=n&&(_stricmp(n,"::1")==0);if(!l&&!v4&&!v6){LogMessage("getaddrinfo('%s')->BLOCKED",n);return EAI_FAIL;}unsigned short p=0;if(s&&*s){char*e=NULL;unsigned long v=strtoul(s,&e,10);if(*s=='\0'||(e&&*e!='\0')||v>65535)return EAI_SERVICE;p=(unsigned short)v;}int fh=h?h->ai_family:AF_UNSPEC;int st=h?h->ai_socktype:0;if(st && st!=SOCK_STREAM && st!=SOCK_DGRAM) return EAI_SOCKTYPE; int w4=v4||(!v6&&(fh==AF_UNSPEC||fh==AF_INET));int w6=v6||(!v4&&(fh==AF_UNSPEC||fh==AF_INET6));PADDRINFOA head=NULL,tail=NULL;if(w6){PADDRINFOA ai=(PADDRINFOA)SAFE_ALLOC(sizeof(ADDRINFOA)+sizeof(struct sockaddr_in6),"getaddrinfo");if(!ai){if(head)ex_freeaddrinfo(head);return EAI_MEMORY;}struct sockaddr_in6*a6=(struct sockaddr_in6*)(ai+1);a6->sin6_family=AF_INET6;a6->sin6_addr=in6addr_loopback;a6->sin6_port=ex_htons(p);ai->ai_family=AF_INET6;ai->ai_socktype= (st==0 ? SOCK_STREAM : st); ai->ai_protocol= (st==SOCK_DGRAM?IPPROTO_UDP:IPPROTO_TCP);ai->ai_addrlen=sizeof(struct sockaddr_in6);ai->ai_addr=(struct sockaddr*)a6;if(!head)head=ai;else tail->ai_next=ai;tail=ai;}if(w4){PADDRINFOA ai=(PADDRINFOA)SAFE_ALLOC(sizeof(ADDRINFOA)+sizeof(struct sockaddr_in),"getaddrinfo");if(!ai){if(head)ex_freeaddrinfo(head);return EAI_MEMORY;}struct sockaddr_in*a=(struct sockaddr_in*)(ai+1);a->sin_family=AF_INET;a->sin_addr.s_addr=ex_inet_addr("127.0.0.1");a->sin_port=ex_htons(p);ai->ai_family=AF_INET;ai->ai_socktype= (st==0 ? SOCK_STREAM : st); ai->ai_protocol=(st==SOCK_DGRAM?IPPROTO_UDP:IPPROTO_TCP);ai->ai_addrlen=sizeof(struct sockaddr_in);ai->ai_addr=(struct sockaddr*)a;if(!head)head=ai;else tail->ai_next=ai;tail=ai;}if(!head)return EAI_FAIL;*r=head;return 0;}
...
int WSAAPI ex_select(int n,fd_set*r,fd_set*w,fd_set*e,const struct timeval*t){UNREFERENCED_PARAMETER(n);EnsureInitialized();DWORD to=t?(t->tv_sec*1000+t->tv_usec/1000):INFINITE;DWORD start=GetTickCount();do{int rc=0;fd_set rr,rw,re;FD_ZERO(&rr);FD_ZERO(&rw);FD_ZERO(&re);Lock();if(r)for(u_int i=0;i<r->fd_count;i++){SHARED_SOCKET*s=FindSharedSocketById(r->fd_array[i]);if(s){BOOL pc=FALSE;if(s->sock_type==SOCK_STREAM && s->is_connected&&s->peer_socket_index!=-1){SHARED_SOCKET*p=&g_pGlobalState->sockets[s->peer_socket_index];if(!p->in_use||(p->shutdown_flags&2))pc=TRUE;}if((s->is_listening&&s->accept_queue_len>0)||(recv_available(s)>0)||pc){FD_SET(s->fake_socket_id,&rr);rc++;}}}if(w)for(u_int i=0;i<w->fd_count;i++){SHARED_SOCKET*s=FindSharedSocketById(w->fd_array[i]);if(s){if(s->sock_type == SOCK_STREAM && s->is_connected&&s->peer_socket_index!=-1){if(recv_free_space(&g_pGlobalState->sockets[s->peer_socket_index])>0){FD_SET(s->fake_socket_id,&rw);rc++;}}else if(s->sock_type == SOCK_DGRAM){FD_SET(s->fake_socket_id,&rw);rc++;}}}if(e){FD_ZERO(e);}Unlock();if(rc>0){if(r)*r=rr;if(w)*w=rw;g_tlsWSAError=0;return rc;}if(to==0)break;Sleep(10);}while(GetTickCount()-start<to);if(r)FD_ZERO(r);if(w)FD_ZERO(w);if(e)FD_ZERO(e);g_tlsWSAError=0;return 0;}
int WSAAPI ex_getsockopt(SOCKET s,int l,int o,char*ov,int*ol){if(!ov||!ol){g_tlsWSAError=WSAEFAULT;return SOCKET_ERROR;}EnsureInitialized();Lock();SHARED_SOCKET*sock=FindSharedSocketById(s);if(!sock){Unlock();g_tlsWSAError=WSAENOTSOCK;return SOCKET_ERROR;}if(l==SOL_SOCKET&&o==SO_ERROR&&*ol>=sizeof(int)){*(int*)ov=g_tlsWSAError;*ol=sizeof(int);g_tlsWSAError=0;}else if(l==SOL_SOCKET&&o==SO_TYPE&&*ol>=sizeof(int)){*(int*)ov=sock->sock_type;*ol=sizeof(int);}else if(l==SOL_SOCKET&&o==SO_LINGER&&*ol>=sizeof(struct linger)){memcpy(ov,&sock->linger_opts,sizeof(struct linger));*ol=sizeof(struct linger);}else if(l==IPPROTO_TCP&&o==TCP_NODELAY&&*ol>=sizeof(int)){*(int*)ov=sock->tcp_nodelay;*ol=sizeof(int);}Unlock();return 0;}
...
int WSAAPI ex_sendto(SOCKET s,const char*b,int l,int f,const struct sockaddr*t,int tl){
    EnsureInitialized(); UNREFERENCED_PARAMETER(f); if (l <= 0) return 0;
    LogMessage("sendto(" PTR_FORMAT ", len=%d)", (UINT_PTR)s, l);
    Lock();
    SHARED_SOCKET* snd = FindSharedSocketById(s);
    if (!snd) { Unlock(); g_tlsWSAError = WSAENOTSOCK; return SOCKET_ERROR; }
    if (snd->sock_type != SOCK_DGRAM) { Unlock(); g_tlsWSAError = WSAEOPNOTSUPP; return SOCKET_ERROR; }
    AutoBindIfNeeded(snd, t->sa_family, SOCK_DGRAM);
    int rcv_idx = -1; unsigned short port = GetSockaddrPort(t);
    for (int i = 0; i < MAX_SHARED_SOCKETS; i++) {
        SHARED_SOCKET* r = &g_pGlobalState->sockets[i];
        if (r->in_use && r->is_bound && r->sock_type == SOCK_DGRAM && r->bind_addr.ss_family == t->sa_family && GetSockaddrPort((struct sockaddr*)&r->bind_addr) == port) {
            rcv_idx = i; break;
        }
    }
    if (rcv_idx == -1) { Unlock(); LogMessage(" -> sendto failed, no listener on port %u", port); g_tlsWSAError = WSAECONNREFUSED; return SOCKET_ERROR; }
    SHARED_SOCKET* rcv = &g_pGlobalState->sockets[rcv_idx];
    size_t packet_size = sizeof(DATAGRAM_PACKET) + l;
    if (recv_free_space(rcv) < packet_size) { Unlock(); LogMessage(" -> sendto would block"); g_tlsWSAError = WSAEWOULDBLOCK; return SOCKET_ERROR; }
    DATAGRAM_PACKET hdr; hdr.len = l;
    if (snd->is_bound) { memcpy(&hdr.from_addr, &snd->bind_addr, snd->bind_addr_len); hdr.from_len = snd->bind_addr_len; }
    else { memset(&hdr.from_addr, 0, sizeof(hdr.from_addr)); hdr.from_len = 0; }
    char* buf = GetRecvBuffer(rcv); size_t cap = recv_capacity(); size_t tail = rcv->recv_tail;
    size_t first = min(sizeof(hdr), cap - tail); memcpy(buf + tail, &hdr, first);
    if (sizeof(hdr) > first) memcpy(((char*)&hdr) + first, buf, sizeof(hdr) - first);
    tail = (tail + sizeof(hdr)) % cap;
    first = min((size_t)l, cap - tail); memcpy(buf + tail, b, first);
    if ((size_t)l > first) memcpy(buf, b + first, l - first);
    rcv->recv_tail = (rcv->recv_tail + l) % cap;
    SOCKET rid = rcv->fake_socket_id;
    Unlock();
    #if ENABLE_DATA_DUMP
    DumpDataToFile("SENDTO", s, b, l);
    #endif
    char ren[96]; MakeReadEventName(ren, sizeof(ren), rid); HANDLE rh = OpenEventA(EVENT_MODIFY_STATE, FALSE, ren); if (rh) { SetEvent(rh); CloseHandle(rh); }
    return l;
}
int WSAAPI ex_recvfrom(SOCKET s,char*b,int l,int f,struct sockaddr*fr,int*fl){
    EnsureInitialized(); if (l <= 0) return 0;
    char ren[96]; MakeReadEventName(ren, sizeof(ren), s); HANDLE h = CreateEventA(NULL, FALSE, FALSE, ren);
    if (!h) { g_tlsWSAError = WSAENETDOWN; return SOCKET_ERROR; }
    while (1) {
        Lock();
        SHARED_SOCKET* rcv = FindSharedSocketById(s);
        if (!rcv) { Unlock(); CloseHandle(h); g_tlsWSAError = WSAENOTSOCK; return SOCKET_ERROR; }
        if (rcv->sock_type != SOCK_DGRAM) { Unlock(); CloseHandle(h); g_tlsWSAError = WSAEOPNOTSUPP; return SOCKET_ERROR; }
        if (recv_available(rcv) >= sizeof(DATAGRAM_PACKET)) {
            DATAGRAM_PACKET hdr; char* buf = GetRecvBuffer(rcv); size_t cap = recv_capacity(); size_t head = rcv->recv_head;
            size_t first = min(sizeof(hdr), cap - head); memcpy(&hdr, buf + head, first);
            if (sizeof(hdr) > first) memcpy(((char*)&hdr) + first, buf, sizeof(hdr) - first);
            if (recv_available(rcv) >= sizeof(DATAGRAM_PACKET) + hdr.len) {
                int bytes_to_read = min(l, hdr.len);
                size_t data_start = (head + sizeof(DATAGRAM_PACKET)) % cap;
                first = min((size_t)bytes_to_read, cap - data_start);
                memcpy(b, buf + data_start, first);
                if ((size_t)bytes_to_read > first) memcpy(b + first, buf, bytes_to_read - first);
                if (fr && fl) { int from_len = min(*fl, hdr.from_len); memcpy(fr, &hdr.from_addr, from_len); *fl = hdr.from_len; }
                if (!(f & MSG_PEEK)) { rcv->recv_head = (rcv->recv_head + sizeof(DATAGRAM_PACKET) + hdr.len) % cap; }
                Unlock();
                CloseHandle(h);
                #if ENABLE_DATA_DUMP
                if (!(f & MSG_PEEK)) DumpDataToFile("RECVFROM", s, b, bytes_to_read);
                #endif
                LogMessage("recvfrom(" PTR_FORMAT ", len=%d) -> read %d", (UINT_PTR)s, l, bytes_to_read);
        ...
        Unlock();
        if (nb) { CloseHandle(h); g_tlsWSAError = WSAEWOULDBLOCK; return SOCKET_ERROR; }
        LogMessage(" -> recvfrom(" PTR_FORMAT ") waiting", (UINT_PTR)s);
        WaitForSingleObject(h, INFINITE);
    }
}

int WSAAPI ex_WSAEnumProtocolsA(LPINT lpiProtocols, LPVOID lpProtocolBuffer, LPDWORD lpdwBufferLength) {
    LogMessage("WSAEnumProtocolsA called");
    if (!lpdwBufferLength) {
        ex_WSASetLastError(WSAEFAULT);
        return SOCKET_ERROR;
    }

    // Підтримуємо тільки TCP та UDP
    WSAPROTOCOL_INFOA supported_protocols[] = {
        {
            .dwServiceFlags1 = XP1_GUARANTEED_DELIVERY | XP1_GUARANTEED_ORDER | XP1_GRACEFUL_CLOSE,
            .iAddressFamily = AF_INET, .iSocketType = SOCK_STREAM, .iProtocol = IPPROTO_TCP,
            .iMaxSockAddr = sizeof(struct sockaddr_in), .iMinSockAddr = sizeof(struct sockaddr_in),
            .szProtocol = "TCP/IP (EXLOUD EMU)"
        },
        {
            .dwServiceFlags1 = XP1_CONNECTIONLESS | XP1_MESSAGE_ORIENTED,
            .iAddressFamily = AF_INET, .iSocketType = SOCK_DGRAM, .iProtocol = IPPROTO_UDP,
            .iMaxSockAddr = sizeof(struct sockaddr_in), .iMinSockAddr = sizeof(struct sockaddr_in),
            .szProtocol = "UDP/IP (EXLOUD EMU)"
        }
    };
    int num_protocols = sizeof(supported_protocols) / sizeof(supported_protocols[0]);
    DWORD required_size = num_protocols * sizeof(WSAPROTOCOL_INFOA);

    if (lpiProtocols) {
        UNREFERENCED_PARAMETER(lpiProtocols);
    }
    
    if (*lpdwBufferLength < required_size) {
        *lpdwBufferLength = required_size;
        ex_WSASetLastError(WSAENOBUFS);
        return SOCKET_ERROR;
    }

    memcpy(lpProtocolBuffer, supported_protocols, required_size);
    *lpdwBufferLength = required_size;
    ex_WSASetLastError(0);
    return num_protocols;
}

int WSAAPI ex_WSAEnumProtocolsW(LPINT lpiProtocols, LPVOID lpProtocolBuffer, LPDWORD lpdwBufferLength) {
    LogMessage("WSAEnumProtocolsW called");
    if (!lpdwBufferLength) {
        ex_WSASetLastError(WSAEFAULT);
        return SOCKET_ERROR;
    }
    
    DWORD ansi_buffer_size = 0;
    ex_WSAEnumProtocolsA(lpiProtocols, NULL, &ansi_buffer_size);
    if (ansi_buffer_size == 0) {
        *lpdwBufferLength = 0;
        return 0;
    }
    
    WSAPROTOCOL_INFOA* ansi_info = (WSAPROTOCOL_INFOA*)SAFE_ALLOC(ansi_buffer_size, "EnumProtocolsW_temp");
    if (!ansi_info) {
        ex_WSASetLastError(WSAENOBUFS);
        return SOCKET_ERROR;
    }
    
    int num_protocols = ex_WSAEnumProtocolsA(lpiProtocols, ansi_info, &ansi_buffer_size);
    if (num_protocols == SOCKET_ERROR) {
        SAFE_FREE(ansi_info);
        return SOCKET_ERROR;
    }

    DWORD required_size_w = num_protocols * sizeof(WSAPROTOCOL_INFOW);
    if (*lpdwBufferLength < required_size_w) {
        *lpdwBufferLength = required_size_w;
        SAFE_FREE(ansi_info);
        ex_WSASetLastError(WSAENOBUFS);
        return SOCKET_ERROR;
    }
    
    WSAPROTOCOL_INFOW* wide_info = (WSAPROTOCOL_INFOW*)lpProtocolBuffer;
    for (int i = 0; i < num_protocols; ++i) {
        memcpy(&wide_info[i], &ansi_info[i], sizeof(WSAPROTOCOL_INFOA) - (sizeof(ansi_info[i].szProtocol)));
        MultiByteToWideChar(CP_ACP, 0, ansi_info[i].szProtocol, -1, wide_info[i].szProtocol, WSAPROTOCOL_LEN);
    }
