 ...
 */
#ifndef NETWORK_ONLINE
#define NETWORK_ONLINE          0
#endif
 ...
 *
 * Returns:
 *   FALSE - Destination is NOT reachable (always)
 */
BOOL APIENTRY IsDestinationReachableA(LPCSTR lpszDestination, LPQOCINFO lpQOCInfo)
{
    ...
    if (lpQOCInfo) {
        ...
        if (lpQOCInfo->dwSize >= sizeof(QOCINFO)) {
            lpQOCInfo->dwFlags    = 0;
    ...
    case DLL_PROCESS_ATTACH:
    {
        ...
        if (!AttachConsole(ATTACH_PARENT_PROCESS)) {
            if (AllocConsole()) {
                ...
                SetConsoleTitleA("SENSAPI Emulator");
            }
        } else {
            FILE* fDummy;
            freopen_s(&fDummy, "CONOUT$", "w", stdout);
