 ...
 *   - HostDomain (REG_SZ)       - Domain/workgroup name  
 *   - MACAddress (REG_BINARY)   - 6 bytes MAC address
 * 
 * Run setup.bat to configure registry before using!
 * ============================================================================ */

#define REGISTRY_ROOT               "Software\\EXLOUD\\Config"
#define REG_COMPUTER_NAME           "ComputerName"
#define REG_HOST_DOMAIN             "HostDomain"
#define REG_PRIMARY_DNS             "PrimaryDNS"
#define REG_SECONDARY_DNS           "SecondaryDNS"
#define REG_PRIMARY_WINS            "PrimaryWINS"
#define REG_SECONDARY_WINS          "SecondaryWINS"
#define REG_SCOPE_ID                "ScopeID"
#define REG_MAC_ADDRESS             "MACAddress"

#define MAX_COMPUTER_NAME_LEN       256
#define MAC_ADDRESS_SIZE            6

// Types removed for Windows SDK compatibility

// NET_IF_COMPARTMENT_SCOPE defined in SDK
...
#define IPHLPAPI_DEBUG  0   /* <-- CHANGE HERE: 1 = Debug, 0 = Production */
 ...
 * ============================================================================
 * Set to 1 to simulate working internet connection
 * Set to 0 to simulate NO internet (local network only)
 * ============================================================================ */

#define SIMULATE_INTERNET_CONNECTION  0   /* <-- CHANGE HERE: 1 = Internet, 0 = No Internet */
    ...
    #define LOG_MSG_FMT(fmt, ...)   LogMessage(fmt, ##__VA_ARGS__)  /* Alias for LOG_MSG */
...
/* 
 * MAC Address - SYNCHRONIZED with netapi32.dll
 * 
 * CRITICAL: Loaded from registry on first use
 * Registry: HKEY_CURRENT_USER\Software\EXLOUD\Config\MACAddress (REG_BINARY)
 * 
 * NO HARDCODED VALUE! Must be set via setup.bat
 */
static BYTE ADAPTER_MAC[6] = {0};
 ...
 */
static BOOL ReadRegistryString(const char* valueName, char* buffer, DWORD bufferSize)
{
    HKEY hKey;
    DWORD dwType = REG_SZ;
    ...
    result = RegOpenKeyExA(HKEY_CURRENT_USER, REGISTRY_ROOT, 0, KEY_READ, &hKey);
    if (result != ERROR_SUCCESS) {
        LOG_MSG("  -> Registry key not found: %s", REGISTRY_ROOT);
        return FALSE;
    }
    
    result = RegQueryValueExA(hKey, valueName, NULL, &dwType, (LPBYTE)buffer, &dwSize);
    RegCloseKey(hKey);
    
    if (result != ERROR_SUCCESS || dwType != REG_SZ) {
        LOG_MSG("  -> Registry value not found: %s", valueName);
        return FALSE;
    }
 ...
 * Returns TRUE on success, FALSE on failure
 */
static BOOL ReadRegistryBinary(const char* valueName, BYTE* buffer, DWORD bufferSize)
{
    HKEY hKey;
    ...
    result = RegQueryValueExA(hKey, valueName, NULL, &dwType, buffer, &dwSize);
    RegCloseKey(hKey);
    
    if (result != ERROR_SUCCESS || dwType != REG_BINARY) {
        LOG_MSG("  -> Registry value not found or wrong type: %s", valueName);
        return FALSE;
    }
 ...
 * Called on first use, loads MAC address and other config
 * Returns TRUE on success, FALSE if registry not configured
 */
static BOOL EnsureConfigLoaded(void)
{
    if (g_ConfigLoaded) return TRUE;
    
    LOG_MSG("Loading configuration from registry: %s", REGISTRY_ROOT);
    
    /* Load MAC address from registry */
    if (!ReadRegistryBinary(REG_MAC_ADDRESS, ADAPTER_MAC, 6)) {
        LOG_MSG("  -> ERROR: MAC address not found in registry!");
        LOG_MSG("  -> Run setup.bat to configure registry");
        return FALSE;
    }
    
    LOG_MSG("  -> MAC Address: %02X:%02X:%02X:%02X:%02X:%02X (from registry)",
...
/* ============================================================================
 * СТАТИЧНЕ ЛІНКУВАННЯ З EXWS2.DLL (ntohl/htonl)
 * ============================================================================
 * Використовуємо статичне лінкування замість динамічного завантаження.
 * При компіляції підключається exws2.lib через CMake
 * ============================================================================ */
...
/* Імпорт функцій з exws2.dll через статичне лінкування */
__declspec(dllimport) ULONG WSAAPI ntohl(ULONG netlong);
__declspec(dllimport) ULONG WSAAPI htonl(ULONG hostlong);

#ifdef __cplusplus
}
#endif

/* Функції ntohl() та htonl() тепер доступні напряму */
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID lpReserved)
{
    ...
    switch (reason) {
        case DLL_PROCESS_ATTACH:
            ...
            /* exws2.dll лінкується статично */
            break;
            
        case DLL_PROCESS_DETACH:
            LOG_MSG("DLL_PROCESS_DETACH");  /* ← Додано! */
            CleanupLogging();
            break;
    }
...
DWORD WINAPI GetIfEntry(PMIB_IFROW pIfRow)
{
    ...
    /* Ensure configuration is loaded from registry */
    if (!EnsureConfigLoaded()) {
        LOG_MSG("  -> ERROR: Configuration not found in registry!");
...
#if SIMULATE_INTERNET_CONNECTION
    pIfRow->dwOperStatus = MIB_IF_OPER_STATUS_OPERATIONAL;
    LOG_MSG(" -> OK, Status=OPERATIONAL (with internet)");
#else
    pIfRow->dwOperStatus = MIB_IF_OPER_STATUS_NON_OPERATIONAL;
    LOG_MSG(" -> OK, Status=NON_OPERATIONAL (no internet)");
#endif
...
DWORD WINAPI GetIfEntry2(PMIB_IF_ROW2 Row)
{
    ...
    LOG_MSG(" -> OK, OperStatus=Up, MediaConnectState=Connected (with internet)");
#else
    Row->OperStatus = IfOperStatusDormant;
    Row->MediaConnectState = MediaConnectStateDisconnected;
    LOG_MSG(" -> OK, OperStatus=Dormant, MediaConnectState=Disconnected (no internet)");
#endif
...
DWORD WINAPI GetIpAddrTable(PMIB_IPADDRTABLE pIpAddrTable, PULONG pdwSize, BOOL bOrder)
{
    ...
    LOG_MSG(" -> OK, IP=%s (with internet)", Ipv4ToString(IPV4_ADDRESS));
#else
    pIpAddrTable->table[0].dwAddr = 0;
    pIpAddrTable->table[0].dwMask = 0;
    pIpAddrTable->table[0].dwBCastAddr = 0;
    LOG_MSG(" -> OK, IP=0.0.0.0 (no internet)");
#endif
...
DWORD WINAPI GetUnicastIpAddressTable(ADDRESS_FAMILY Family, PMIB_UNICASTIPADDRESS_TABLE *Table)
{
    ...
    /* IP адреси є завжди (локальна мережа) */
    DWORD count = 0;
...
ULONG WINAPI GetAdaptersInfo(PIP_ADAPTER_INFO AdapterInfo, PULONG SizePointer)
{
    ...
    WideCharToMultiByte(CP_ACP, 0, ADAPTER_DESCRIPTION, -1,
                        AdapterInfo->Description, sizeof(AdapterInfo->Description),
                        NULL, NULL);
    ...
    lstrcpyA(AdapterInfo->IpAddressList.IpAddress.String, "0.0.0.0");
    lstrcpyA(AdapterInfo->IpAddressList.IpMask.String, "0.0.0.0");
    lstrcpyA(AdapterInfo->GatewayList.IpAddress.String, "0.0.0.0");
    lstrcpyA(AdapterInfo->GatewayList.IpMask.String, "0.0.0.0");
    LOG_MSG(" -> OK, IP=0.0.0.0, GW=None (no internet)");
#endif

    /* ======================================================================
     * WINS Servers (NEW in v2.0 - Wine compatible)
     * ====================================================================== */
    char primaryWINS[16] = {0};
    char secondaryWINS[16] = {0};
    BOOL hasPrimaryWINS = ReadRegistryString(REG_PRIMARY_WINS, primaryWINS, sizeof(primaryWINS));
    BOOL hasSecondaryWINS = ReadRegistryString(REG_SECONDARY_WINS, secondaryWINS, sizeof(secondaryWINS));
    
    if (hasPrimaryWINS && primaryWINS[0]) {
        lstrcpyA(AdapterInfo->PrimaryWinsServer.IpAddress.String, primaryWINS);
        lstrcpyA(AdapterInfo->PrimaryWinsServer.IpMask.String, "255.255.255.255");
        AdapterInfo->PrimaryWinsServer.Context = 0;
        AdapterInfo->HaveWins = TRUE;
        LOG_MSG_FMT("  Primary WINS: %s", primaryWINS);
        
        if (hasSecondaryWINS && secondaryWINS[0]) {
            lstrcpyA(AdapterInfo->SecondaryWinsServer.IpAddress.String, secondaryWINS);
            lstrcpyA(AdapterInfo->SecondaryWinsServer.IpMask.String, "255.255.255.255");
            AdapterInfo->SecondaryWinsServer.Context = 0;
            LOG_MSG_FMT("  Secondary WINS: %s", secondaryWINS);
        }
    } else {
        AdapterInfo->HaveWins = FALSE;
        LOG_MSG("  WINS: Not configured");
    }
    ...
    PIP_ADAPTER_ADDRESSES AdapterAddresses, PULONG SizePointer)
{
    ...
    /* Додатковий простір для 2 DNS серверів (завжди виділяємо, навіть якщо не використовуємо) */
    ULONG size = sizeof(IP_ADAPTER_ADDRESSES) + 512 + (2 * sizeof(IP_ADAPTER_DNS_SERVER_ADDRESS_XP));
    ...
    /* Адаптер завжди Up (локальна мережа працює) */
...
#if SIMULATE_INTERNET_CONNECTION
    /* Симулюємо DNS сервери Google (8.8.8.8 та 8.8.4.4) */
    PIP_ADAPTER_DNS_SERVER_ADDRESS_XP DnsServer1 = (PIP_ADAPTER_DNS_SERVER_ADDRESS_XP)ptr;
    ptr = (BYTE*)ptr + sizeof(IP_ADAPTER_DNS_SERVER_ADDRESS_XP);
    
    DnsServer1->Length = sizeof(IP_ADAPTER_DNS_SERVER_ADDRESS_XP);
    DnsServer1->Reserved = 0;
    
    struct sockaddr_in* addr1 = (struct sockaddr_in*)&DnsServer1->Address;
    addr1->sin_family = AF_INET;
    addr1->sin_addr.s_addr = htonl(0x08080808); /* 8.8.8.8 */
    DnsServer1->Address.iSockaddrLength = sizeof(struct sockaddr_in);
    
    PIP_ADAPTER_DNS_SERVER_ADDRESS_XP DnsServer2 = (PIP_ADAPTER_DNS_SERVER_ADDRESS_XP)ptr;
    ptr = (BYTE*)ptr + sizeof(IP_ADAPTER_DNS_SERVER_ADDRESS_XP);
    
    DnsServer2->Length = sizeof(IP_ADAPTER_DNS_SERVER_ADDRESS_XP);
    DnsServer2->Reserved = 0;
    DnsServer2->Next = NULL;
    
    struct sockaddr_in* addr2 = (struct sockaddr_in*)&DnsServer2->Address;
    addr2->sin_family = AF_INET;
    addr2->sin_addr.s_addr = htonl(0x08080404); /* 8.8.4.4 */
    DnsServer2->Address.iSockaddrLength = sizeof(struct sockaddr_in);
    
    DnsServer1->Next = DnsServer2;
    AdapterAddresses->FirstDnsServerAddress = DnsServer1;
    
    AdapterAddresses->DnsSuffix = (WCHAR*)ptr;
    lstrcpyW((WCHAR*)ptr, L"");
    
    *SizePointer = size;
    LOG_MSG("  -> OK, OperStatus=Up, DNS=8.8.8.8,8.8.4.4 (internet)");
#else
    /* ======================================================================
     * DNS Servers (NEW in v2.0)
     * ====================================================================== */
    char primaryDNS[16] = {0};
    char secondaryDNS[16] = {0};
    BOOL hasPrimaryDNS = ReadRegistryString(REG_PRIMARY_DNS, primaryDNS, sizeof(primaryDNS));
    BOOL hasSecondaryDNS = ReadRegistryString(REG_SECONDARY_DNS, secondaryDNS, sizeof(secondaryDNS));
    
    if (hasPrimaryDNS && primaryDNS[0]) {
        /* Виділяємо пам'ять для DNS серверів після структури */
        PIP_ADAPTER_DNS_SERVER_ADDRESS_XP pDnsServer1 = 
            (PIP_ADAPTER_DNS_SERVER_ADDRESS_XP)((BYTE*)AdapterAddresses + sizeof(IP_ADAPTER_ADDRESSES) + 512);
        
        pDnsServer1->Length = sizeof(IP_ADAPTER_DNS_SERVER_ADDRESS_XP);
        pDnsServer1->Reserved = 0;
        pDnsServer1->Next = NULL;
        
        struct sockaddr_in* addr1 = (struct sockaddr_in*)&pDnsServer1->Address;
        addr1->sin_family = AF_INET;
        inet_pton(AF_INET, primaryDNS, &addr1->sin_addr);
        pDnsServer1->Address.iSockaddrLength = sizeof(struct sockaddr_in);
        
        AdapterAddresses->FirstDnsServerAddress = (PIP_ADAPTER_DNS_SERVER_ADDRESS)pDnsServer1;
        LOG_MSG_FMT("  Primary DNS: %s", primaryDNS);
        
        /* Другий DNS якщо є */
        if (hasSecondaryDNS && secondaryDNS[0]) {
            PIP_ADAPTER_DNS_SERVER_ADDRESS_XP pDnsServer2 = 
                (PIP_ADAPTER_DNS_SERVER_ADDRESS_XP)((BYTE*)pDnsServer1 + sizeof(IP_ADAPTER_DNS_SERVER_ADDRESS_XP));
            
            pDnsServer2->Length = sizeof(IP_ADAPTER_DNS_SERVER_ADDRESS_XP);
            pDnsServer2->Reserved = 0;
            pDnsServer2->Next = NULL;
            
            struct sockaddr_in* addr2 = (struct sockaddr_in*)&pDnsServer2->Address;
            addr2->sin_family = AF_INET;
            inet_pton(AF_INET, secondaryDNS, &addr2->sin_addr);
            pDnsServer2->Address.iSockaddrLength = sizeof(struct sockaddr_in);
            
            pDnsServer1->Next = (PIP_ADAPTER_DNS_SERVER_ADDRESS)pDnsServer2;
            LOG_MSG_FMT("  Secondary DNS: %s", secondaryDNS);
        }
    } else {
        AdapterAddresses->FirstDnsServerAddress = NULL;
        LOG_MSG("  DNS: Not configured");
    }
...
DWORD WINAPI GetIpForwardTable(PMIB_IPFORWARDTABLE table, PULONG size, BOOL sort)
{
    LOG_CALL_FMT("Size=%lu", size ? *size : 0);
    (void)sort;
    if (!size) return ERROR_INVALID_PARAMETER;

#if SIMULATE_INTERNET_CONNECTION
    const DWORD route_count = 2; // local + default
#else
    const DWORD route_count = 0; // isolated
#endif

    ULONG needed = sizeof(MIB_IPFORWARDTABLE) +
                   (route_count > 0 ? (route_count - 1) * sizeof(MIB_IPFORWARDROW) : 0);
    ...
    ZeroMemory(table, needed);
    table->dwNumEntries = route_count;

#if SIMULATE_INTERNET_CONNECTION
    // route 0: local subnet (direct)
    table->table[0].dwForwardDest    = (IPV4_ADDRESS & IPV4_NETMASK);
    table->table[0].dwForwardMask    = IPV4_NETMASK;
    table->table[0].dwForwardPolicy  = 0;
    table->table[0].dwForwardNextHop = 0; // direct
    table->table[0].dwForwardType    = MIB_IPROUTE_TYPE_DIRECT;
    table->table[0].dwForwardProto   = MIB_IPPROTO_LOCAL;
    table->table[0].dwForwardAge     = 0;
    table->table[0].dwForwardIfIndex = ADAPTER_IF_INDEX;
    table->table[0].dwForwardNextHopAS = 0;
    table->table[0].dwForwardMetric1 = 25;
    table->table[0].dwForwardMetric2 = 0;
    table->table[0].dwForwardMetric3 = 0;
    table->table[0].dwForwardMetric4 = 0;
    table->table[0].dwForwardMetric5 = 0;

    // Wine‑style: for direct route, replace NextHop with interface address
    if (!table->table[0].dwForwardNextHop)
        table->table[0].dwForwardNextHop = IPV4_ADDRESS;

    // route 1: default via gateway
    const DWORD IPV4_GATEWAY = 0x0101A8C0; // 192.168.1.1 (host-order for Ipv4ToString)
    table->table[1].dwForwardDest    = 0;
    table->table[1].dwForwardMask    = 0;
    table->table[1].dwForwardPolicy  = 0;
    table->table[1].dwForwardNextHop = IPV4_GATEWAY;
    table->table[1].dwForwardType    = MIB_IPROUTE_TYPE_INDIRECT;
    table->table[1].dwForwardProto   = MIB_IPPROTO_NETMGMT;
    table->table[1].dwForwardAge     = 0;
    table->table[1].dwForwardIfIndex = ADAPTER_IF_INDEX;
    table->table[1].dwForwardNextHopAS = 0;
    table->table[1].dwForwardMetric1 = 25;
    table->table[1].dwForwardMetric2 = 0;
    table->table[1].dwForwardMetric3 = 0;
    table->table[1].dwForwardMetric4 = 0;
    table->table[1].dwForwardMetric5 = 0;

    LOG_MSG(" -> OK, NumEntries=2 (local + default route)");
#else
    LOG_MSG(" -> OK, NumEntries=0 (isolated, NO routes)");
#endif
...
DWORD WINAPI GetBestRoute(DWORD dwDestAddr, DWORD dwSourceAddr, PMIB_IPFORWARDROW pBestRoute)
{
    ...
    /* Локальні адреси маршрутизуються завжди */
...
#if SIMULATE_INTERNET_CONNECTION
    /* З інтернетом - додати default gateway тут */
    LOG_MSG("  -> ERROR_HOST_UNREACHABLE (non-local, gateway not implemented)");
    return ERROR_HOST_UNREACHABLE;
#else
    /* Без інтернету - зовнішні адреси недоступні */
    LOG_MSG("  -> ERROR_NETWORK_UNREACHABLE (no default gateway)");
...
DWORD WINAPI GetNetworkConnectivityHint(NL_NETWORK_CONNECTIVITY_HINT *ConnectivityHint)
{
...
#if SIMULATE_INTERNET_CONNECTION
    ConnectivityHint->ConnectivityLevel = NetworkConnectivityLevelHintInternetAccess;
    ConnectivityHint->ConnectivityCost = NetworkConnectivityCostHintUnrestricted;
    LOG_MSG("  -> InternetAccess (full connectivity)");
#else
    ...
    LOG_MSG("  -> LocalAccess (local network only, no internet)");
#endif
...
ULONG WINAPI GetTcpStatisticsEx2(PMIB_TCPSTATS2 Statistics, ULONG Family)
{
    LOG_CALL_FMT("Family=%lu", Family);
    if (!Statistics) return ERROR_INVALID_PARAMETER;
    ZeroMemory(Statistics, sizeof(MIB_TCPSTATS2));
    Statistics->RtoAlgorithm = 4;
    Statistics->dwRtoMin = 300;
    Statistics->dwRtoMax = 120000;
    Statistics->dwMaxConn = 0xFFFFFFFF;
    return NO_ERROR;
...
ULONG WINAPI GetUdpStatisticsEx2(PMIB_UDPSTATS2 Statistics, ULONG Family)
{
    LOG_CALL_FMT("Family=%lu", Family);
    if (!Statistics) return ERROR_INVALID_PARAMETER;
    ZeroMemory(Statistics, sizeof(MIB_UDPSTATS2));
    return NO_ERROR;
}
...
/* ============================================================================
 * GetNetworkParams - REGISTRY-BASED (v2.0)
 * ============================================================================
 * 
 * This function reads configuration from Windows Registry:
 *   HKEY_CURRENT_USER\Software\EXLOUD\Config
 * 
 * Registry values:
 *   - ComputerName (REG_SZ) → HostName
 *   - HostDomain (REG_SZ) → DomainName
 * 
 * NO FALLBACKS! If registry keys don't exist, returns ERROR_FILE_NOT_FOUND.
 * Run setup.bat to create registry configuration.
 * 
 * SYNCHRONIZED with netapi32.dll - both read from same registry location.
 * ============================================================================ */
...
DWORD WINAPI GetNetworkParams(PFIXED_INFO pFixedInfo, PULONG pOutBufLen)
{
    ...
    /* Базовий розмір + місце для другого DNS */
    ULONG size = sizeof(FIXED_INFO) + sizeof(IP_ADDR_STRING);
    ...
    /* ======================================================================
     * Hostname і Domain (обов'язкові)
     * ====================================================================== */
    if (!ReadRegistryString(REG_COMPUTER_NAME, pFixedInfo->HostName, sizeof(pFixedInfo->HostName))) {
        LOG_MSG("  -> ERROR: ComputerName not found in registry!");
        LOG_MSG("  -> Run add_to_reg.bat to configure");
        return ERROR_FILE_NOT_FOUND;
    }
    
    if (!ReadRegistryString(REG_HOST_DOMAIN, pFixedInfo->DomainName, sizeof(pFixedInfo->DomainName))) {
        LOG_MSG("  -> ERROR: HostDomain not found in registry!");
        LOG_MSG("  -> Run add_to_reg.bat to configure");
        return ERROR_FILE_NOT_FOUND;
    }
    
    LOG_MSG_FMT("  HostName: %s", pFixedInfo->HostName);
    LOG_MSG_FMT("  DomainName: %s", pFixedInfo->DomainName);
    ...
    if (hasPrimaryDNS && primaryDNS[0]) {
        lstrcpyA(pFixedInfo->DnsServerList.IpAddress.String, primaryDNS);
        lstrcpyA(pFixedInfo->DnsServerList.IpMask.String, "255.255.255.255");
        pFixedInfo->DnsServerList.Context = 0;
        pFixedInfo->DnsServerList.Next = NULL;
        LOG_MSG_FMT("  Primary DNS: %s", primaryDNS);
        
        /* Другий DNS сервер якщо є */
        if (hasSecondaryDNS && secondaryDNS[0]) {
            PIP_ADDR_STRING pSecondary = (PIP_ADDR_STRING)((BYTE*)pFixedInfo + sizeof(FIXED_INFO));
            lstrcpyA(pSecondary->IpAddress.String, secondaryDNS);
            lstrcpyA(pSecondary->IpMask.String, "255.255.255.255");
            pSecondary->Context = 0;
            pSecondary->Next = NULL;
            pFixedInfo->DnsServerList.Next = pSecondary;
            LOG_MSG_FMT("  Secondary DNS: %s", secondaryDNS);
        }
        
        pFixedInfo->CurrentDnsServer = &pFixedInfo->DnsServerList;
    } else {
        /* Default DNS якщо не налаштовано */
        lstrcpyA(pFixedInfo->DnsServerList.IpAddress.String, "8.8.8.8");
        lstrcpyA(pFixedInfo->DnsServerList.IpMask.String, "255.255.255.255");
        pFixedInfo->CurrentDnsServer = &pFixedInfo->DnsServerList;
        LOG_MSG("  DNS: Using default 8.8.8.8");
    }
    
    /* ======================================================================
     * ScopeID (NEW in v2.0 - Wine compatible)
     * ====================================================================== */
    char scopeID[256] = {0};
    if (ReadRegistryString(REG_SCOPE_ID, scopeID, sizeof(scopeID)) && scopeID[0]) {
        lstrcpyA(pFixedInfo->ScopeId, scopeID);
        LOG_MSG_FMT("  ScopeID: %s", scopeID);
    } else {
        pFixedInfo->ScopeId[0] = '\0';
    }
    
    /* ======================================================================
     * Інші параметри
     * ====================================================================== */
    pFixedInfo->NodeType = HYBRID_NODETYPE;  /* 8 = H-node (Wine compatible) */
    pFixedInfo->EnableRouting = 0;
    pFixedInfo->EnableProxy = 0;
    pFixedInfo->EnableDns = 1;
    
    *pOutBufLen = size;
    LOG_MSG("  -> OK (EXLOUD Config v2.0)");
    return NO_ERROR;
}
...
DWORD WINAPI GetIfEntry2Ex(MIB_IF_ENTRY_LEVEL Level, PMIB_IF_ROW2 Row)
{
...
NET_IF_COMPARTMENT_ID WINAPI GetJobCompartmentId(HANDLE JobHandle)
{
    LOG_CALL();
    (void)JobHandle;
    return 1;
