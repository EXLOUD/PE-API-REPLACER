 ...
 * All values are loaded from HKEY_CURRENT_USER\Software\EXLOUD\Config
 * 
...
static void EnsureInit(void) {
    if (InterlockedCompareExchange(&g_initCount, 1, 0) == 0) {
         ...
         * REGISTRY-BASED CONFIGURATION (v2.0)
         * 
         * Load all settings from HKEY_CURRENT_USER\Software\EXLOUD\Config
        ...
        case NCBASTAT: {
            if (!ncb->ncb_buffer || ncb->ncb_length < sizeof(ADAPTER_STATUS)) {
                ...
                if (g_macAddress[0] == 0 && g_macAddress[1] == 0 && g_macAddress[2] == 0 &&
                     ...
                     * Both loaded from registry: HKCU\Software\EXLOUD\Config\MACAddress
                     */
