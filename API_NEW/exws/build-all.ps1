#!/usr/bin/env pwsh
# ============================================================================
# EXWS (wsock32.dll proxy) Build Script
# ============================================================================
# Pure proxy DLL - forwards to exws2.dll and EXMSW.dll
# ============================================================================
...
# Check for resource compiler (llvm-rc or windres)
...
if ($missing) {
    ...
    Write-Host ""
    Write-Host "Install LLVM toolchain with resource compiler" -ForegroundColor Yellow
    exit 1
}
...
function Build-Architecture {
    ...
    if (Test-Path $dllPath) {
        ...
        Write-Status "  Size: $size KB (pure proxy)" -Color Green
    } else {
...
Write-Status "EXWS Build (wsock32.dll Proxy)" -Color Cyan
Write-Status "========================================" -Color Cyan
Write-Status "Runtime: $RuntimeDir"
Write-Status "Toolchain: clang-cl + lld-link"
Write-Status "Type: Pure Proxy (→ exws2.dll + EXMSW.dll)"
...
if ($failCount -gt 0) {
    ...
    Write-Host "  3. Verify files: dllmain.c, ws_32.def, version.rc" -ForegroundColor Yellow
    exit 1
} else {
    Write-Success "All builds completed successfully!"
    Write-Host ""
    Write-Status "Note: This is a pure proxy DLL" -Color Yellow
    Write-Status "Requires: exws2.dll and EXMSW.dll" -Color Yellow
    exit 0
}
